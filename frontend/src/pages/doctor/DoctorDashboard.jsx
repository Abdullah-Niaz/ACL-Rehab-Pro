...
export default function DoctorDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    // High risk: Any patient with active scar tissue complications OR quad symmetry < 75%
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card-premium">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Flexion & ROM Progress Curves</h3>
                <p className="text-xs text-slate-500">Aggregated historical metrics from patients logs.</p>
              </div>
            </div>
            <TrendLine data={chartData} keyName="flexion" label="Knee Flexion" strokeColor="#2563EB" />
          </div>

          <div className="card-premium">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Daily Recovery Trends</h3>
                <p className="text-xs text-slate-500">Logs scoring metrics representing average confidence.</p>
              </div>
            </div>
            <TrendBar data={chartData} keyName="recovery" label="Recovery Score" barColor="#10B981" />
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          {stats.highRiskCount > 0 && (
            <div className="card-premium border-red-100 bg-red-50/10">
              <h3 className="font-bold text-red-800 text-sm uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <AlertCircle size={16} />
                High Risk Alerts
              </h3>
              <div className="space-y-2.5">
                {stats.highRiskList.map(p => (
                  <Link 
                    ...
                    className="flex justify-between items-center bg-white p-3 rounded-xl border border-red-100 hover:border-red-300 transition"
                  >
                    <div>
                      <p className="font-bold text-xs text-slate-900">{p.user?.name}</p>
          ...
          <div className="card-premium">
            ...
            {recentActivities.length === 0 ? (
              <p className="text-xs text-slate-500 italic">No check-in logs submitted today.</p>
            ) : (
                ...
                {recentActivities.map((log) => {
                  ...
                  return (
                    <div key={log._id} className="relative text-xs">
                      {/* Timeline dot */}
                      <span className="absolute -left-[21px] top-1 h-2.5 w-2.5 rounded-full bg-blue-600 ring-4 ring-white"></span>
                      ...
                      {log.exerciseLogs && log.exerciseLogs[0] && (
                        <div className="mt-1.5 p-2 bg-slate-50 rounded-lg text-[10px] text-slate-600">
